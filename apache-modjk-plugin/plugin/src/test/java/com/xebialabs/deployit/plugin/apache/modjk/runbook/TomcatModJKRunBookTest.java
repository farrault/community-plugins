...
public class TomcatModJKRunBookTest extends BaseTestForModJKRunBook {
	...
	public void setup() throws IOException {
		...
		TomcatServer as3 = new TomcatUnmanagedServer();
		as3.setLabel("Tomcatas3");
		as3.setAjpPort(8009);
		as3.setHost(host2);

		TomcatServer as4 = new TomcatUnmanagedServer();
	...
	@Test
	public void testConfigurationWithJkManager3URLPrefix() throws Exception {
		mapping.getSource().setUrlUnmounts(("/*.css"));
		mapping.getSource().setUrlMounts("    /myURL,     /foo  ");
		deployModJkConfiguration(deployment);
		assertContains(getModJKConfiguration(), "JkLogFile", "JkShmFile", "JkLogLevel", "JkLogStampFormat");
		assertNotContains(getModJKConfiguration(), "JkOptions");
		assertNotContains(getModJKConfiguration(), "JkOptions");
		...
		final HostSession session = apacheHttpdServer.connectToAdminHost();
		assertContains(getVirtualHostConfigFilePath(session, apacheHttpdServer, mapping), "JkMount /myURL LB-my.pet.local_8890");
		assertContains(getVirtualHostConfigFilePath(session, apacheHttpdServer, mapping), "JkUnMount /*.css LB-my.pet.local_8890");
		assertContains(getVirtualHostConfigFilePath(session, apacheHttpdServer, mapping), "JkMount /foo LB-my.pet.local_8890");
		session.close();
	...
	@Test
	public void testWithWorkerSpacesInName() throws Exception {
		jkConfiguration.setStickySession(false);
		for (Serializable s : deployment.getTarget().getMembers()) {
			int x = 0;
			if (s instanceof TomcatServer) {
				TomcatServer tomcat = (TomcatServer) s;
				tomcat.setLabel("My Tomcat Server on AZERTY" + (++x));
			}
		}
		deployModJkConfiguration(deployment);
		assertContains(apacheHttpdServerModJk.getJkWorkerFile(), "worker.My.Tomcat.Server.on.AZERTY1.type=ajp13");
		assertContains(apacheHttpdServerModJk.getJkWorkerFile(), "worker.LB-my.pet.local_8890.balance_workers=My.Tomcat.Server.on.AZERTY1");
